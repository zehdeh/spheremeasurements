...
class MainWindow(VTKMainWindow):
	...
	def setupErrorGrid(self,stereoCameras, renderer, gridSize, gridScale):
		...
		alphaChannelFunc.AddPoint(0.0, 0.1)
		alphaChannelFunc.AddPoint(1.0, 0.9)
		...
		print np.max(self.errorMatrix)
		print np.min(self.errorMatrix)
		dataArray = numpy_to_vtk(num_array=self.errorMatrix.ravel(), array_type=vtk.VTK_FLOAT)
		...
		rectilinearGrid = vtk.vtkRectilinearGrid()
		rectilinearGrid.SetDimensions(50,50,50)
		...
		points = vtk.vtkPoints()
		rectilinearGrid.GetPoints(points)
		ugrid.SetPoints(points)
		ugrid.GetPointData().SetScalars(dataArray)

		for i in range(rectilinearGrid.GetNumberOfCells()):
			cell = rectilinearGrid.GetCell(i)
		...
		self.volumeMapper.SetInputConnection(trifilter.GetOutputPort())
		self.volumeMapper.Update()
		print ugrid
		#self.volumeMapper.SetInputConnection(self.imageImport.GetOutputPort())
